...
def normalize_html(data: bytes, name: str) -> bytes:
    text = data.decode('utf-8'); lower = text.lower()
    if lower.count('<title>') > 1:
        body_end = lower.find('</body>')
        if body_end < 0: raise RuntimeError(f'{name}: duplicated document without closing body')
        text = text[: body_end + len('</body>')] + '</html>'
    else:
        end = lower.find('</html>')
        if end >= 0: text = text[: end + len('</html>')]
    ...
    def hub_replacement(match: re.Match[str]) -> str:
        raw_label = match.group(1); lookup = html.unescape(raw_label)
        slug = HUB_LABEL_TO_SLUG.get(lookup) or NORMALIZED_LABEL_TO_SLUG.get(normalized_label(lookup))
        if slug is None: raise RuntimeError(f'{name}: unknown hotel hub thumbnail label: {raw_label}')
        return f'<img alt="{raw_label}" loading="lazy" src="/assets/hotels/{slug}/batch-thumb.svg">'
    ...
    if name in HUBS and 'practical-title-above' not in text:
        ...
        if replaced != 1: raise RuntimeError(f'{name}: could not move practical title above ten-base grid')
    normalized = text.lower()
    if normalized.count('<title>') != 1: raise RuntimeError(f'{name}: expected one title after normalization')
    if normalized.count('<h1') != 1: raise RuntimeError(f'{name}: expected one h1 after normalization')
    if normalized.count('rel="canonical"') != 1: raise RuntimeError(f'{name}: expected one canonical after normalization')
    if '<span class="batch-thumb"' in normalized: raise RuntimeError(f'{name}: obsolete sprite span still present')
    for slug in re.findall(r'src="/assets/hotels/([^/]+)/batch-thumb\.svg"', text):
        if not (ROOT / 'assets' / 'hotels' / slug / 'batch-thumb.svg').is_file():
...
def main() -> int:
    ...
    if not part_files: raise RuntimeError('Hotel hub payload parts not found')
    ...
    with tarfile.open(fileobj=io.BytesIO(raw), mode='r:gz') as archive:
        ...
        if bad: raise RuntimeError(f'Unexpected hotel hub payload paths: {bad}')
        for member in members:
            target = ROOT / member.name; target.parent.mkdir(parents=True, exist_ok=True)
            source = archive.extractfile(member)
            if source is None: raise RuntimeError(f'Cannot read payload member: {member.name}')
            data = source.read()
            if member.name.endswith('.html'): data = normalize_html(data, member.name)
            target.write_bytes(data); print(f'Materialized {member.name}')
