...
('<li><a href="/restaurants/">Manger &amp; faire</a></li>','<li><a href="/restaurants/">Explorer</a></li>'),
...
for rel,pairs in repls.items():
 ...
 for old,new in pairs:
  if old not in text: raise SystemExit(f'Expected text not found in {rel}: {old[:80]}')
  text=text.replace(old,new)
 if text!=original: p.write_text(text,encoding='utf-8'); changed.append(rel)
