...
BASES = {
    ...
    "cannes": {
        ...
        "fr_new": "Treize hôtels, trois façons de faire Cannes.",
        "en_old": "Seven hotels, three ways to do Cannes.",
        "en_new": "Thirteen hotels, three ways to do Cannes.",
    },
    "villefranche": {
    ...
    "beaulieu": {
        ...
        "fr_new": "Cinq hôtels pour profiter d’une base plus calme",
        "en_old": "Three hotels for a calmer base",
        "en_new": "Five hotels for a calmer base",
    },
    "mougins": {
...
HOTELS = [
    ...
    dict(slug="canopy-by-hilton-cannes", display="Canopy by Hilton Cannes", url="https://expedia.com/affiliates/cannes-hotels-canopy-by-hilton-cannes.DMnljs4", base="cannes", fr_section="luxe-central", en_section="central-luxury", fr_tag="Suquet & rooftop", en_tag="Suquet & rooftop", fr_copy="Un luxe contemporain côté Suquet, avec rooftop et moins de révérence pour la Croisette.", en_copy="Contemporary luxury by Le Suquet, with a rooftop and less reverence for Croisette ceremony."),
    dict(slug="hotel-barriere-le-gray-d-albion", display="Hôtel Barrière Le Gray d’Albion", url="https://expedia.com/affiliates/cannes-hotels-hotel-barriere-le-gray-dalbion.r7DqylJ", base="cannes", fr_section="luxe-central", en_section="central-luxury", fr_tag="Centre & plage", en_tag="Central & beach", fr_copy="Très central, très cannois, sans monter jusqu’au cérémonial des trois grands palaces.", en_copy="Very central and very Cannes, without going all the way into grand-palace ceremony."),
    ...
    dict(slug="mondrian-cannes", display="Mondrian Cannes", url="https://expedia.com/affiliates/cannes-hotels-le-grand-hotel-cannes.XuIy6W0", base="cannes", fr_section="luxe-central", en_section="central-luxury", fr_tag="Croisette contemporaine", en_tag="Contemporary Croisette", fr_copy="Une lecture plus contemporaine de la Croisette, avec la plage et le centre déjà dans l’équation.", en_copy="A more contemporary take on the Croisette, with beach and centre already built into the stay."),
    ...
    dict(slug="hotel-le-provencal", display="Hôtel Le Provençal", url="https://expedia.com/affiliates/nice-hotels-hotel-le-provencal.ANkbtuC", base="villefranche", fr_section="port-sans-voiture", en_section="harbour-car-free", fr_tag="Village & simple", en_tag="Village & straightforward", fr_copy="Une base simple dans Villefranche pour privilégier le village, la gare et la baie.", en_copy="A straightforward Villefranche base that puts the village, station and bay first."),
    dict(slug="le-versailles", display="Le Versailles", url="https://expedia.com/affiliates/nice-hotels-le-versailles.RujrwMc", base="villefranche", fr_section="port-sans-voiture", en_section="harbour-car-free", fr_tag="Vue & hauteur", en_tag="View & hillside", fr_copy="Un peu en hauteur, avec piscine et vue : moins collé au port, toujours clairement à Villefranche.", en_copy="A little higher up, with pool and view: less glued to the harbour, still unmistakably Villefranche."),

    dict(slug="columbus-hotel-monte-carlo", display="Columbus Hotel Monte-Carlo", url="https://expedia.com/affiliates/monaco-hotels-columbus-monte-carlo.sY6SwZa", base="monaco", fr_section="resort-plus-detendu", en_section="resort-more-relaxed", fr_tag="Fontvieille & détendu", en_tag="Fontvieille & relaxed", fr_copy="Fontvieille pour une version plus calme et moins cérémonielle de Monaco.", en_copy="Fontvieille for a calmer, less ceremonial version of Monaco."),
    dict(slug="fairmont-monte-carlo", display="Fairmont Monte Carlo", url="https://expedia.com/affiliates/monaco-hotels-fairmont-monte-carlo.Hj2nm3O", base="monaco", fr_section="palace-classique", en_section="classic-palace", fr_tag="Grand luxe central", en_tag="Central grand luxury", fr_copy="Grand format, central et très Monaco : un choix évident si vous voulez que l’adresse fasse partie du séjour.", en_copy="Large-scale, central and unmistakably Monaco: an obvious pick when the address is part of the experience."),
    ...
    dict(slug="princess-et-richmond", display="Princess et Richmond", url="https://expedia.com/affiliates/monaco-hotels-princess-et-richmond.7PV6zac", base="menton", fr_section="riviera-plus-chic", en_section="riviera-more-polished", fr_tag="Mer & plus chic", en_tag="Sea & more polished", fr_copy="Une option plus soignée face à la mer, sans quitter le rythme lent qui fait l’intérêt de Menton.", en_copy="A more polished sea-facing option without losing the slower rhythm that makes Menton worthwhile."),
...
def card_markup(hotel: dict[str, str], lang: str, index: int) -> str:
    ...
    url = html.escape(hotel["url"], quote=True)
    tag = html.escape(hotel[f"{lang}_tag"], quote=True)
    copy = html.escape(hotel[f"{lang}_copy"], quote=False)
    cta = "Voir les tarifs sur Expedia" if lang == "fr" else "Check rates on Expedia"
    slug = hotel["slug"]
    return (
        ...
        f'<span class="batch-thumb batch-thumb-13" role="img" aria-label="{display}" '
        f'style="background-position:{x}% {y}%"></span></a>'
